...
veil_color = ['n','o','w','y']
veil_color_txt = st.selectbox("veil-color", veil_color)
if veil_color_txt == 'w':
    veil_color_no = 2
elif veil_color_txt == 'o':
    veil_color_no = 1
elif veil_color_txt == 'n':
    veil_color_no = 0
elif veil_color_txt == 'y':
    veil_color_no = 4
...
input = [bruises_no,gill_spacing_no,gill_size_no, gill_color_no,stalk_root_no,stalk_surface_above_ring_no,veil_color_no,ring_type_no, spore_print_color_no, population_no]


with open(r"mushroom.pkl", "rb")as file:
    model = pickle.load(file)
    result = None
    res = model.predict([input])
    if st.button("Submit"):
        if res[0] == 0:
            result = "Mushroom is Edible"
        elif res[0] == 1:
            result = "Mushroom is Poisonous "
    st.text(result)
